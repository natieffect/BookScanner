...
class ScanMultiple:
    ...
    def startMain(self):
        ...
        layoutLeft = [ [pysimpleGUI.B('First Capture',key=self.config.get('WINDOW_MAIN','MAIN_BUTTON_LEFT_KEY'))],
        ...
        layoutRight = [ [pysimpleGUI.B('Second Capture',key=self.config.get('WINDOW_MAIN','MAIN_BUTTON_RIGHT_KEY'))],
                        [pysimpleGUI.Image(size=(300,300),key=self.config.get('WINDOW_MAIN','MAIN_IMAGE_RIGHT_KEY'))],
                        [pysimpleGUI.T('Second Alignment',key=self.config.get('WINDOW_MAIN','MAIN_TEXT_RIGHT_KEY'))],
                    ]
        layoutMain = [
            [pysimpleGUI.Column(layoutLeft),pysimpleGUI.VSeperator(),pysimpleGUI.Column(layoutRight)]
        ]

        window = pysimpleGUI.Window(self.config.get('GENERAL_WINDOW','WINDOW_TITLE'),layoutMain,finalize=True,resizable=True)
        ...
        try:
            if streamVideoOne.initializeImutilsRead():
                while True:
                    ...
                    window[self.config.get('WINDOW_MAIN','MAIN_IMAGE_RIGHT_KEY')].update(data=byteFrameOne)
